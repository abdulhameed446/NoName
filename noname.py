...
class GoogleEnum(enumratorBaseThreaded):
    ...
    def should_sleep(self):
        time.sleep(random.randint(5, 10))  # Random delay between 5 and 10 seconds
        return
...
class DNSdumpster(enumratorBaseThreaded):
    ...
    def get_csrftoken(self, resp):
        ...
        if not token:
            if self.verbose:
                print(R + "[!] Error: CSRF token not found for DNSdumpster" + W)
            return None
...
class Virustotal(enumratorBaseThreaded):
    def __init__(self, domain, subdomains=None, q=None, silent=False, verbose=True):
        ...
        self.api_key = "YOUR_VIRUSTOTAL_API_KEY"  # Add your API key here
        return

    def send_req(self, url):
        headers = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36',
            'x-apikey': self.api_key,  # Add API key to headers
        }
        try:
            resp = self.session.get(url, headers=headers, timeout=self.timeout)
        except Exception as e:
    ...
    def enumerate(self):
        while self.url != '':
            ...
            if 'links' in resp and 'next' in resp['links']:
                ...
                self.url = ''
            self.extract_domains(resp)
            self.should_sleep()  # Add delay between requests
        return self.subdomains
